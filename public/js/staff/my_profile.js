document.addEventListener("DOMContentLoaded", function() {
            ...
            drawerEl.addEventListener('shown.bs.offcanvas', function() {
                        ...
                        fetch(`${BASE_URL}/staff/main/my-profile/get`)
                            .then(res => res.json())
                            .then(data => {
                                    if (data.error) { content.innerHTML = `<div class="text-danger text-center p-5">${data.error}</div>`; return; }

                                    content.innerHTML = `
    <form action="${BASE_URL}/staff/main/my-profile/update" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3"><label class="formal-label">Phone Number</label>
            <input type="text" name="phone" class="formal-input" value="${data.phone || ''}"></div>
        ${data.position ? `<div class="mb-4"><label class="formal-label">Position</label><input type="text" class="formal-input read-only-input" value="${data.position}" readonly></div>` : ''}
