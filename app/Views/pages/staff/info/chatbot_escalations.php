    ...
    <div id="content">
        ...
        <div class="container-fluid p-4" style="font-size: 12px;">
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    <a href="?status=escalated" class="text-decoration-none kpi-filter-link">
            ...
            <div class="alert alert-light border d-flex justify-content-between align-items-center mb-3" style="font-size: 11px;">
                <span><strong>
                    <?php
                        $labels = ['escalated' => 'Awaiting Response', 'in_progress' => 'In Progress', 'resolved' => 'Resolved'];
            ...
            <div class="custom-table-container">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        <thead class="table-dark"><tr><th class="ps-4">Conversation</th><th>From</th><th>Query</th><th>Last Activity</th><th>Status</th><th class="text-center">Action</th></tr></thead>
                        <tbody>
                            ...
                            <tr>
                                <td class="ps-4">#CONV-<?= str_pad($e['conversation_id'], 4, '0', STR_PAD_LEFT) ?></td>
                                <td class="fw-bold"><?= esc($e['customer_name'] ?: 'Guest') ?> <small class="text-muted">(<?= esc($e['customer_role'] ?? 'guest') ?>)</small></td>
                                <td><small class="text-muted"><?= esc(substr($e['first_query'] ?? 'No message recorded', 0, 40)) ?></small></td>
                                <td><?= date('M d, h:i A', strtotime($e['updated_at'])) ?></td>
                                <td><span class="badge <?= $e['status']=='escalated'?'bg-danger':($e['status']=='in_progress'?'bg-warning text-dark':'bg-success') ?> px-3"><?= strtoupper(str_replace('_',' ',$e['status'])) ?></span></td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-dark rounded-pill px-4 btn-join-chat" data-id="<?= $e['conversation_id'] ?>">
